...
int main(int argc,char *argv[])
{
  ...
  for (;;) {
	...
	jy=nx*ny-1;
      ...
      if (c==',' || c=='.') {
	iy = (int) floor((double)jx / (double)nx);
	ix = jx - iy * nx;
	jy = iy + ny * ix;
      }
      if (c=='<' || c=='>' || c=='\t' || c=='\b') {
	ix = (int) floor((double)jy / (double)ny);
	iy = jy - ix * ny;
	jx = ix + nx * iy;
      }
...
void dec2sex(double x,char *s,int f,int len)
{
  ...
  if (len==0) sprintf(s,"%02i%c%02i",(int) deg,form[f][0],(int) min);  
